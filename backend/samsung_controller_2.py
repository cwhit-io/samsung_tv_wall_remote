...
# --- Configuration ---
TV_INFO_FILE = 'tv_info.json'
TV_KEYS_FILE = 'tv_keys.json'
...
def load_tv_keys():
    ...
    return {
        'power-on': 'WOL', 'power-off': 'KEY_POWER', 'volup': 'KEY_VOLUP', 'voldown': 'KEY_VOLDOWN', 
        'mute': 'KEY_MUTE', 'chup': 'KEY_CHUP', 'chdown': 'KEY_CHDOWN', 'menu': 'KEY_MENU', 
        'home': 'KEY_HOME', 'source': 'KEY_SOURCE', 'guide': 'KEY_GUIDE', 'up': 'KEY_UP', 
        'down': 'KEY_DOWN', 'left': 'KEY_LEFT', 'right': 'KEY_RIGHT', 'enter': 'KEY_ENTER', 
        'return': 'KEY_RETURN', 'hdmi1': 'KEY_HDMI1', 'hdmi2': 'KEY_HDMI2', 'hdmi3': 'KEY_HDMI3', 
        'hdmi4': 'KEY_HDMI4'
    }
...
def send_wol_packet(mac_address, broadcast_ip='255.255.255.255'):
    ...
    try:
        ...
        if len(mac_bytes) != 6: raise ValueError("Invalid MAC")
...
def get_token(tv_ip, force_pairing=False):
    ...
    ws = None
    try:
        ws = websocket.create_connection(uri, timeout=60, sslopt={"cert_reqs": ssl.CERT_NONE})
...
def send_command(tv_ip, command_key, token=None):
    """Sends a single command key to the TV via WebSocket."""
    if not get_token(tv_ip): return False, "Authentication token not available"
    uri = f"wss://{tv_ip}:8002/api/v2/channels/samsung.remote.control?name={APP_NAME_ENCODED}&token={get_token(tv_ip)}"
    ws = None
    try:
        ws = websocket.create_connection(uri, timeout=5, sslopt={"cert_reqs": ssl.CERT_NONE})
        response = json.loads(ws.recv())
        ...
        payload = {"method": "ms.remote.control", "params": {"Cmd": "Click", "DataOfCmd": command_key, "Option": "false", "TypeOfRemote": "SendRemoteKey"}}
...
def handle_power_on(tv_ip):
    ...
    if is_tv_on(tv_ip):
        return {"ip": tv_ip, "name": name, "success": True, "message": "TV is already on."}
    
    tv_info = get_tv_info(tv_ip)
    mac = tv_info.get('mac')
    if not mac:
        return {"ip": tv_ip, "name": name, "success": False, "message": "No MAC address configured."}
    
    tv_broadcast_ip = '10.10.111.255' # Your subnet's broadcast IP
    ...
    for _ in range(10): 
        time.sleep(3)
        if is_tv_on(tv_ip):
            return {"ip": tv_ip, "name": name, "success": True, "message": "TV successfully powered on."}
    
    return {"ip": tv_ip, "name": name, "success": True, "message": "WOL sent, but verification timed out. TV may be booting slowly."}
...
def handle_power_off(tv_ip):
    ...
    if not is_tv_on(tv_ip):
        return {"ip": tv_ip, "name": name, "success": True, "message": "TV is already off (unresponsive to ping)."}
    ...
    if success:
        return {"ip": tv_ip, "name": name, "success": True, "message": "Power-off command sent successfully."}
    else:
        # This will catch issues like a bad token or network error.
        return {"ip": tv_ip, "name": name, "success": False, "message": f"Failed to send power-off command: {msg}"}
...
def main():
    ...
    parser.add_argument('--command', required=True, choices=list(tv_keys.keys()), help="Command to send.")
    parser.add_argument('--target', required=True, help="Target TV IP(s), comma-separated or 'all'.")
    ...
    if args.list_commands:
        ...
        for cmd, key in sorted(tv_keys.items()): print(f"  {cmd}: {key}")
        return
    if args.list:
        tv_info = load_tv_info()
        print("\n===== CONFIGURED TVs =====")
    ...
    targets = get_all_tvs() if args.target == 'all' else [ip.strip() for ip in args.target.split(',')]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(targets) or 1) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_ip):
            try:
                ...
                results.append({"ip": ip, "name": get_tv_name(ip), "success": False, "message": str(exc)})
    ...
    if successful:
        ...
        for r in sorted(successful, key=lambda x: x['name']): print(f"  ✓ {r['name']} ({r['ip']}): {r['message']}")
    if failed:
        print("\nFailed Operations:")
        for r in sorted(failed, key=lambda x: x['name']): print(f"  ✗ {r['name']} ({r['ip']}): {r['message']}")
